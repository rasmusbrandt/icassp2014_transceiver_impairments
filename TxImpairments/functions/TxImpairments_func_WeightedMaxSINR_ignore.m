...
function user_rates_e = ...
  TxImpairments_func_WeightedMaxSINR_ignore(H,C,D,Pt,sigma2r,Nd,stop_crit,imp_params,weighted_maxsinr,MS_oblivious)
  ...
  while (iter <= MAX_ITERS)
    ...
    [T,G] = TxImpairments_func_T_G(H,U,Wident,C,D);
    ...
    for l = 1:Kt
      ...
      if(Kc > 0)
        ...
        for k = Dl
          ...
          for n = 1:Nd
            % Build interference plus noise covariance matrix, for this stream
            Brs = T(:,:,l) - (G(:,n,k)*G(:,n,k)') + sigma2r*eye(Mt);
          ...
          V(:,:,k) = sqrt(Pt/Kc)*V(:,:,k)/norm(V(:,:,k),'fro');
        end
      end
